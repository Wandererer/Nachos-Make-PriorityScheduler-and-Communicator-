...
public class PriorityScheduler extends Scheduler {
    ...
    private static class FirstStartTest implements Runnable {	//우선순위 역전 현상을 보여주기 위해서 우선순위가 낮고, lock을 가지고 있는 스레드를 한번 실행시켜 주기 위한 테스트용 클래스	  
    	...
    	public void run() {
    		...
    		ThreadedKernel.scheduler.setPriority(two, 5);	//lock을 요청하는 스레드인 two의 최초 우선순위를 5로 설정	  
    		ThreadedKernel.scheduler.setPriority(three, 5);	//lock을 요청하지 않는 스레드인 three의 최초 우선순위를 5로 설정	  
    ...
    public static void selfTest() {	//ThreadedKernel.java에서 selfTest()안에서 수행을 시켜줄 부분.	실질적으로 여기서 테스트가 시작됨	  
    	...
    	FirstStartTest testThread1 = new FirstStartTest("testThread 1", mutex);		//제일 먼저 한번 실행 시켜줄 testThread1 생성
